...
func TestIntegrationRecirculateWaitingHumanClearsQuestions(t *testing.T) {
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200, got %d: %s", resp.StatusCode, readBodyStr(resp))
	}
	...
	body := strings.TrimSpace(readBodyStr(getResp))
	if body != "[]" {
...
func TestSmokeQuestionEndpointsViaRealServer(t *testing.T) {
	...
	emptyBody := strings.TrimSpace(readBodyStr(resp))
	resp.Body.Close()
	if emptyBody != "[]" {
	...
	if resp.StatusCode != http.StatusCreated {
		t.Errorf("POST question: expected 201, got %d: %s", resp.StatusCode, readBodyStr(resp))
	}
	var created QuestionResponse
	...
	if resp.StatusCode != http.StatusOK {
		t.Errorf("PATCH answer: expected 200, got %d: %s", resp.StatusCode, readBodyStr(resp))
	}
	resp.Body.Close()
