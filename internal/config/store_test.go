...
// postgresTestDSN uses a dedicated test DB for the config package. The config
// store tests truncate provider/tier tables which conflicts with internal/db
// tests when sharing devteam_test_db. Using devteam_test_config isolates them.
const postgresTestDSN = "host=localhost port=5432 user=devteam password=devteam dbname=devteam_test_config sslmode=disable"
