...
// TestAC008_Migration020Idempotent verifies migration 020 is idempotent: running
...
func TestAC008_Migration020Idempotent(t *testing.T) {
	d := setupTestDB(t)

	// Drop the config tables so we can re-run the migration from scratch against
	// this connection's schema (the migration runner already applied 020 once at
	...
	runOnce := func() {
		...
		if err := migration020ProviderConfig(tx); err != nil {
			tx.Rollback()
			t.Fatalf("migration020 Up: %v", err)
		}
...
// TestAC008_Migration020SeedValues verifies the seed row values are correct:
...
func TestAC008_Migration020SeedValues(t *testing.T) {
	d := setupTestDB(t)
...
// seedProviderConfigForTest re-inserts the migration 020 seed rows after
// truncateAllTables wiped them. Tests that rely on the seed call this. The seed
// mirrors migration_020.go exactly; if the migration changes, update both.
