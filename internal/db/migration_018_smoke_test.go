...
// TestMigration018_Applies verifies migration 018 creates the
// repo_operation_config table with the correct columns and no FK.
func TestMigration018_Applies(t *testing.T) {
	d := setupTestDB(t)
	// setupTestDB already ran migrations (including 018) on connect.
	// Verify columns.
