...
// TestMigration019_RoundTrip verifies migration 019 applies additively and
// idempotently: empty DB → migrate → all v1 tables/columns/indexes present;
// re-run migrate → no error (S-MIG-01, S-MIG-02).
func TestMigration019_RoundTrip(t *testing.T) {
	d := setupTestDB(t)
	// setupTestDB already runs all pending migrations (including 019) via Open.
	...
	// see 19 as applied and skip it. Verify by opening a second connection
	...
	defer d2.Close()
	// No panic, no error — migration 019 was skipped as already-applied.
	if !tableExists(t, d2, "feature_defaults") {
		t.Error("feature_defaults missing after second Open (idempotency check)")
	}
}

// TestMigration019_ActorNullable verifies the actor column is nullable so
// legacy rows (and the backward-compat RecordAuditEvent overload) survive
// (S-MIG-03 AC1).
func TestMigration019_ActorNullable(t *testing.T) {
	d := setupTestDB(t)
	fid := "feat-mig019-actor"
	seedFeature(t, d, fid)
...
// TestMigration019_AdditiveOnly verifies the schema diff vs the pre-019
...
// every pre-019 column (the only change is the additive actor column).
func TestMigration019_AdditiveOnly(t *testing.T) {
	d := setupTestDB(t)

	// audit_events pre-019 columns are still present.
