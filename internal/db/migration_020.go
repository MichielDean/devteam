...
func init() {
	RegisterMigration(Migration{
		...
		Up:      migration020ProviderConfig,
	})
}

// migration020ProviderConfig creates the provider/tier config tables, seeds the
...
func migration020ProviderConfig(tx *sql.Tx) error {
	statements := []string{
		...
		// ON CONFLICT DO NOTHING: re-running migration 020 (if ever re-applied) is a no-op.
	...
	for _, stmt := range statements {
		if _, err := tx.Exec(stmt); err != nil {
			return fmt.Errorf("migration 020 executing statement: %w\nSQL: %s", err, stmt)
		}
	}
	return nil
}