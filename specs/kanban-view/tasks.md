...
**Tests**: Included — the spec mandates e2e (AC-001..AC-022) and a unit test (AC-011).

**Organization**: Tasks grouped by user story priority (P1: US-001 + US-002, P2: US-003, P3: US-004), with a foundational phase for shared primitives and a cross-cutting phase for regression + full e2e.

## Format: `[ID] [P?] [Story] Description`

- **[P]**: Can run in parallel (different files, no dependencies)
- **[Story]**: US1, US2, US3, US4, or SHARED (foundational)
- Exact file paths in descriptions. All paths relative to the worktree root: `~/source/devteam/worktrees/kanban-view/devteam/`.
...
- [ ] T001 [SHARED] Extract `statusColors` map to shared module
  - Files:
    - `ui/src/components/badgeColors.ts` — CREATE (if not present). Export `const statusColors: Record<string, string>` (the 9-entry map: in_progress, done, cancelled, draft, gate_blocked, passed, failed, recirculated, waiting_for_human).
  ...
  - Done conditions:
    ...
    - No other definition of `statusColors` in `ui/src/` (grep `const statusColors` → exactly one match, in `badgeColors.ts`).
    - `npm run dev` loads the Dashboard (click List toggle manually); existing `FeatureCard` renders identically — no visual drift.
...
- [ ] T002 [SHARED] [P] Create `useSessionView` hook — **List default (FR-003)**
  - Files:
    - `ui/src/hooks/useSessionView.ts` — CREATE (or MODIFY if worktree version exists — **it does, and returns `'board'`; MUST change to `'list'`**).
  - Spec: returns `['list' | 'board', (v: 'list' | 'board') => void]`. Lazy-init from `sessionStorage.getItem('devteam.dashboard.view')`; validate value ∈ {'list','board'}, else default **`'list'`** (FR-003). On change, `setItem`. try/catch around storage access → fall back to `'list'` on error (private mode).
  - Constraints: FR-002, FR-003.
  - Done conditions:
    - `useSessionView.ts` exports `useSessionView`.
    - Fresh session (no stored value) → returns **`'list'`** (FR-003, AC-005). **NOT `'board'`.**
    - After `setView('board')`, `sessionStorage.getItem('devteam.dashboard.view')` === `'board'` (FR-002).
    - Invalid stored value (e.g. `'garbage'`) → returns `'list'`.
  ...
  - Agent failure mode checks:
    ...
    - [ ] **Default is `'list'` — verify by reading the source; the worktree's `'board'` default is a spec violation.**

- [ ] T003 [SHARED] [P] Implement and unit-test `groupFeaturesByPhase`
  - Files:
    - `ui/src/components/groupFeaturesByPhase.ts` — CREATE. Pure function.
    - `ui/src/components/groupFeaturesByPhase.test.ts` — CREATE. Vitest unit test.
    - `ui/package.json` — MODIFY (if needed). Ensure `vitest` in devDependencies and `"test:unit": "vitest run"` script. Worktree already has both.
  ...
  - Done conditions:
    ...
    - Test cases (AC-011):
      ...
      - Mixed input → partition sum invariant: `Object.values(groups).reduce((n, f) => n + f.length, 0) === input.length`.
      - One feature per known phase → each bucket length 1, `other` length 0 (SC-002).
...
**Checkpoint**: Foundational primitives ready — `badgeColors`, `useSessionView` (List default), `groupFeaturesByPhase` all exist and tested. User-story implementation can begin.

---

## Phase 2: User Story 1 — Toggle Between List and Kanban Board (Priority: P1) 🎯 MVP

**Goal**: A user can switch between List and Board views; choice persists for the session; **List is default**.

**Independent Test**: Load Dashboard → List is active by default (AC-001/005). Click "Board" → columns render (AC-002). Click "List" → FeatureList renders (AC-003). Reload → Board persists (AC-004).

### Implementation for User Story 1

- [ ] T004 [US1] [P] Create `ViewToggle` component
  - Files:
    - `ui/src/components/ViewToggle.tsx` — CREATE (or verify worktree version).
  - Spec: two `<button>` elements, container `data-testid="view-toggle"`, buttons `data-testid="view-toggle-list"` / `"view-toggle-board"`. Active button `aria-pressed="true"`, inactive `aria-pressed="false"`. Props `{ view: 'list' | 'board'; onViewChange: (v) => void }`. Tailwind: blue active, gray inactive, `dark:` variants.
  - Constraints: FR-001.
  - Done conditions:
...
- [ ] T005 [US1] Wire toggle + Board/List conditional into `Dashboard.tsx`
  - Files:
    - `ui/src/pages/Dashboard.tsx` — MODIFY (worktree already wired; verify + fix comment).
  - Spec:
    - Import `useSessionView`, `ViewToggle`, `KanbanBoard`.
    - `const [view, setView] = useSessionView();`
    - Render `<ViewToggle view={view} onViewChange={setView} />` **only** inside the `!isLoading && !error && features.length > 0` branch (FR-004).
    - In that branch: `view === 'board' ? <KanbanBoard features={features} /> : <FeatureList features={features} />`.
    - Loading / error / empty branches unchanged (FR-017, CON-008).
    - **Fix the misleading comment** ("Board default" → "List default per FR-003").
  - Dependencies: T002 (useSessionView with List default), T004 (ViewToggle), T006 (KanbanBoard component).
  - Constraints: FR-001, FR-002, FR-003, FR-004, FR-016, FR-017, CON-007, CON-008.
  - Done conditions:
    - `npm run dev` — Dashboard loads with **List** visible by default (AC-001/005).
    - Click "Board" → six columns render, zero `feature-list` (AC-002).
    - Click "List" → `FeatureList` renders, zero `kanban-column-*` (AC-003).
    - Reload after selecting Board → Board still active (AC-004).
  ...
  - Agent failure mode checks:
    ...
    - [ ] **Default view is List — `useSessionView` returns `'list'`** (FR-003).

**Checkpoint**: User Story 1 fully functional and independently testable — toggle works, **List default**, persistence, empty/loading/error states correct.
...
- [ ] T006 [US2] Implement `KanbanBoard` component (full)
  - Files:
    - `ui/src/components/KanbanBoard.tsx` — CREATE (or verify worktree version). Re-exports `groupFeaturesByPhase` from `./groupFeaturesByPhase`.
  ...
  - Done conditions:
    ...
    - Board container has `overflow-x-auto` (or `scroll`) — AC-022.
...
- [ ] T007 [US2] Implement `KanbanColumn` component
  - Files:
    - `ui/src/components/KanbanColumn.tsx` — CREATE (or verify worktree version).
...
- [ ] T008 [US2] [P] Implement `KanbanCard` component
  - Files:
    - `ui/src/components/KanbanCard.tsx` — CREATE (or verify worktree version).
...
- [ ] T011 [SHARED] Update `app.spec.ts` for List-default (CON-004 regression safety)
  - Files:
    - `ui/e2e/app.spec.ts` — MODIFY (worktree already has `switchToList` helper — verify it works with List default; with List default the helper is a no-op but harmless).
  - Spec: existing list-view tests assert `feature-card-*` on first load. With **List** now default (FR-003, after T002 fix), those assertions pass without a click. The `switchToList` helper is a defensive no-op (clicks List only if visible) — **keep it** so the tests are robust to a future default change. Only for tests that assert list-view DOM. Tests that assert API behavior (`request.get`) or the count badge are unaffected.
  - Constraints: CON-004.
  - Done conditions:
  ...
  - Agent failure mode checks:
    ...
    - [ ] `switchToList` helper waits for `view-toggle-list` to be visible (handles loading state) — or no-ops cleanly when already on List.
    - [ ] Tests that skip on empty workspace still skip correctly.

- [ ] T012 [SHARED] Create `kanban.spec.ts` covering AC-001..AC-022 — **assert List default**
  - Files:
    - `ui/e2e/kanban.spec.ts` — CREATE (or MODIFY worktree version — **it currently asserts Board default; MUST change AC-001/005 to assert List default**).
  - Spec: one test per acceptance criterion (or grouped where one test covers multiple ACs):
    - AC-001: toggle visible, **List** active by default (`view-toggle-list[aria-pressed="true"]`).
    - AC-002: click Board → 6 columns, no feature-list.
    - AC-003: click List → feature-list, no columns.
    - AC-004: reload after selecting Board → Board persists (sessionStorage).
    - AC-005: fresh context → **List** default.
    ...
    - AC-011: (unit — already in T003; e2e T019 verifies board surfaces `other` column).
  ...
  - Done conditions:
    ...
    - **AC-001 and AC-005 assert `view-toggle-list[aria-pressed="true"]` — NOT `view-toggle-board`** (FR-003).
  ...
  - Agent failure mode checks:
    ...
    - [ ] **AC-001/005 assert List default — worktree currently asserts Board default (spec violation, MUST fix).**
...
- **Phase 1 (Foundational)**: no dependencies — start immediately. T001, T002, T003 can run in parallel (different files).
...
- **Phase 6 (Cross-cutting)**: T011 (app.spec.ts) depends on T005. T012 (kanban.spec.ts) depends on T005/T006/T007/T008/T009/T010.
...
### Recommended Execution Order (single developer — brownfield reconcile)

**The worktree already has working implementations of every component.** The primary task is reconciling the two spec divergences (List default; test assertions) and verifying against the central spec's 22 ACs.

1. **T002 (useSessionView List default)** — highest priority spec fix. Change `'board'` → `'list'`.
2. **T001, T003, T004, T008** — verify worktree code matches spec; no changes expected.
3. **T007, T006** — verify; no changes expected.
4. **T005** — verify Dashboard wiring + fix misleading comment.
5. **T009, T010** — verify empty/overflow behavior.
6. **T011** — verify `app.spec.ts` `switchToList` helper works with List default.
7. **T012** — **fix kanban.spec.ts AC-001/005 to assert List default**; verify all 22 ACs covered.

## Implementation Strategy

### Brownfield Reconcile (this feature)

1. Fix `useSessionView` default (`'board'` → `'list'`) — T002.
2. Fix `kanban.spec.ts` AC-001/005 assertions (Board → List default) — T012.
3. Verify all other worktree code matches the central spec (six columns, no Backlog, vertical scroll, empty placeholders, terminal features visible, single fetch, no new deps).
4. Run `npm run test:unit` (AC-011) + `npm run test:e2e` (all ACs).
5. Grep checks: CON-005 (no duplicate literals), CON-006 (single `statusColors`), CON-003 (`package.json` deps unchanged).
...
- **Highest-priority fix**: T002 (List default) — the worktree's `'board'` default violates FR-003/AC-001/AC-005.
