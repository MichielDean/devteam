...
> **No change to this endpoint.** The Kanban Board consumes its response read-only. The Board adds zero new endpoints, zero new query params, zero new fetches (CON-007, FR-016). This contract is pinned server-side by `internal/api/kanban_smoke_test.go`.
...
|---|---|---|---|
| `features` | `FeatureSummary[]` | yes | **MUST be `[]` not `null`** when empty (CON-008, pinned by `kanban_smoke_test.go`). Board treats `[]` as the empty-board state → `EmptyState` renders (FR-004). |
...
| `status` | `string` | yes | no | one of `STATUS_LABELS` keys (defensive: unknown tolerated) | Status badge + ring flag (FR-008/011) |
| `priority` | `number` | yes | no | `1 \| 2 \| 3` | Priority badge (FR-008) |
| `current_phase` | `string` | yes | no | one of `PHASES` (defensive: unknown → "Other" column) | Column assignment (FR-006/007) |
| `updated_at` | `string` (ISO 8601) | yes | no | parseable date | Card updated line |
| `gate_result` | `GateResult \| null` | no | yes | — | Gate indicator when present (FR-009) |
| `pending_questions_count` | `number` | yes | no | `>= 0` | Question badge when `> 0` (FR-008) |
...
```json
{
  "features": [
    {
      "id": "feat-1", "title": "Kanban View", "status": "in_progress", "priority": 1,
      "current_phase": "planning", "updated_at": "2026-06-22T16:57:08Z",
      "gate_result": null, "pending_questions_count": 2
    },
    {
      "id": "feat-2", "title": "Count Badge", "status": "done", "priority": 3,
      "current_phase": "delivery", "updated_at": "2026-06-20T10:00:00Z",
...
Board renders: `feat-1` in Planning column (question badge shows `2`); `feat-2` in Delivery column with `✓ Gate passed`. Both visible — terminal status not filtered.
...
Dashboard renders `EmptyState`; toggle hidden (FR-004, AC-006/018). Board does not render.
