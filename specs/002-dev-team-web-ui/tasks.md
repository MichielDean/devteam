...
- [ ] T004 [P] [INFRA] Verify helper methods in `internal/feature/feature.go` and `internal/feature/types.go` — Confirm `IsTerminal() bool` already exists on `Feature` (returns true if Status is Done or Cancelled). Verify JSON tags on all exported types produce correct API serialization. In `internal/feature/types.go`, verify `AllPhases()`, `ValidPhaseNames()`, `IsValidPhase()`, `ParsePhase()`, `String()`, and `ArtifactAPIPathToType()` are sufficient for API validation needs. Add any missing helper methods if gaps are found (e.g., `IsValidPriority()`, phase ordering helpers)
