  ...
  return (
    <div data-testid="kanban-board" className="flex gap-4 overflow-x-auto min-w-max pb-2">
      {columns.map(col => (
        ...
        >
          <header data-testid={`kanban-column-header-${col.phase}`} className="px-3 py-2 border-b border-gray-200 dark:border-gray-700 shrink-0">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">{col.label}</h3>
          </header>
          <div className="flex flex-col gap-2 p-2 flex-1">
            {col.features.length === 0 ? (
              <p data-testid={`kanban-column-empty-${col.phase}`} className="text-xs text-gray-400 dark:text-gray-500 text-center py-4">No features</p>
            ) : (
