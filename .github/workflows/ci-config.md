...
| Migration test | None at CI level (sibling's `migration_017` test runs in `backend-full`) | **Delta — see §4.** Migration 019 round-trip test must run in CI against a fresh schema, not just `backend-full`. |
| Deploy/rollback | `deploy.sh`, `rollback.sh` (recreate, smoke-gated, auto-rollback) | **Unchanged by this stage.** Stage 4.1 owns the CD delta. This artifact records the migration-019 rollback implication for 4.1 (§6). |
...
|-----|---------|--------------|------------------|---------|---------|-----------------------|
| `backend` | Build + vet + db/api/config unit + race | `go build ./...` → `go vet` (widened, §5) → `go test ./internal/db/... ./internal/api/... ./internal/config/... -count=1` → `go test -race ./internal/repos/... ./internal/config/... ./internal/settings/... -count=1` | Go 1.26.1 | postgres:16 | 10m | New `internal/db/migration_019*`, `internal/repos`, `internal/settings/*`, `internal/api/settings_handlers*`, `internal/api/auth_guard*`, `internal/config` write-path tests run here. Vet scope widened (§5); `internal/config` added to fast unit gate (§5.1); `-race` step added (§5.2). |
...
### 4.2 This feature's migration: 019

**Migration 017 is taken on `main`** (`repos_registry`, sibling feature). This feature's migration is **019** (the `unit-of-work` artifact references "017" because it was written before the sibling feature's migration landed; the developer at 3.6 renumbers to 019 to avoid collision, following the same precedent the sibling feature used when it renumbered from 013 to 017). The schema content is **thinned** per `bolt-plan` revision 2 / `test-results` revision 2: only `feature_defaults`, `server_config`, `audit_events.actor`, and `idx_audit_events_type_time` are added. **No `llm_*` tables, no `cicd_platforms` table** — those were removed by the Q7/Q8 scope cut.

### 4.3 CI behavior for migration 019

The migration round-trip tests (`internal/db/migration_019_test.go::TestMigration019_RoundTrip`, `::TestMigration019_AdditiveOnly`, `::TestMigration019_ActorNullable`, `::TestMigration019_NoReposTableAlteration`, `::TestMigration019_Atomicity`, `::TestMigration019_NumberIs019`, `::TestFeatureDefaults_ValidatorMinimal` — per `test-results` §2.1 T-MIG-01..07) run against a **fresh test DB with no `schema_migrations` rows**. In CI, the `backend` and `backend-full` jobs create the test DBs via the existing `for db in devteam_test_db ...` loop, but the test harness's `setupTestDB(t)` applies all registered migrations — which means migration 019 runs as part of `setupTestDB`, not as a fresh-DB test.

**Decision for v1: no new CI step for the migration round-trip test.** Rationale:
1. The round-trip test (`TestMigration019_RoundTrip` and siblings) are **Go test functions** that create their own isolated DB (or truncate `schema_migrations` + all tables) within the test. They run in the existing `backend` and `backend-full` jobs automatically — they're `_test.go` files under `internal/db/`, picked up by `go test ./internal/db/...` and `go test ./...`.
2. Adding a dedicated "fresh DB" CI step would require either (a) a second postgres service container with a separate `CREATE DATABASE` and a custom `go test -run TestMigration019_RoundTrip` invocation, or (b) a test-harness refactor to expose a `setupFreshTestDB(t)` helper. Both are test-infrastructure work whose value is marginal given that the tests already run in the existing jobs — the "fresh DB" isolation is the test's own responsibility, not the CI job's.
...
## 6. Migration 019 Rollback Implication (input to 4.1)
...
- **Migration 019 is forward-only** (no `Down`), per the established convention (R-TEST-03, `team-practices` §6.4).
- **Code rollback** (via `rollback.sh`): `git revert` the merge commit, rebuild, restart. The reverted binary does not know about migration 019's tables — they are orphaned but harmless (additive, <1 MB, ignored by the old code). Same posture as the sibling feature's migration 017.
- **Data rollback** (if migration 019 fails in production): manual `pg_restore` from the last `pg_dump` backup, or a forward-fix migration 020. **No automated data rollback.** This is the inherited posture (`team-practices` §6.3); this feature does not change it.
...
7. **Migration 019 (not 017)** — verified `internal/db/migration_017.go` on `main` is `repos_registry` (sibling feature); this feature's migration is 019. The `unit-of-work` "017" reference is renumbered by the developer at 3.6. ✓
